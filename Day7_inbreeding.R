...
CreateGeneFrequencies <- function(allele_frequencies,inbreeding_coefficient = 0.0)
{
	...
	if (abs(sum(allele_frequencies) - 1.0) > 0.0001)
	{
		print("ERROR");
		print(sum(allele_frequencies))
		print(abs(sum(allele_frequencies) - 1.0))
		print("BREAK");		
	}
...
Exercise5b <-function()
{
	fitnesses <- c(1.0,2.0,0.5)
	inbreeding_coefficient <- 0.0
	initial_allele_frequencies <- c(0.9,0.1)
	
	allele_frequencies <- matrix(0,0,length(initial_allele_frequencies))
