...
py<-0.6; pb<-0.3; po<-1-py-pb   # Probability value at time zero
p<-c(py,pb,po)
generations<-10
hbo<-0.2
hby<-0.3
...
three_alleles<-function(p,w_given,generations){
  ...
  w_alleles<-c((1-p_alleles[2]*(1-hyo))*w_phenotype[1]+p_alleles[2]*w_phenotype[2]-hyo*w_phenotype[3],
               (1-p_alleles[3]*(1-hbo))*w_phenotype[2]+p_alleles[3]*w_phenotype[3]-hbo*w_phenotype[1],
               (1-p_alleles[1]*(1-hby))*w_phenotype[3]+p_alleles[1]*w_phenotype[1]-hby*w_phenotype[2])
...
# TRIPLOT FOR ALLELES
#triplot(results_sl[,c(6,5,4)],label=c("O","B","Y"),pch=20,col=4, main="Alleles 1996 fitness")
#trilines(results_sl[,c(6,5,4)], col=4)
#triplot(results_s[,c(6,5,4)],label=c("O","B","Y"),pch=20,col=4, main="Alleles 2001 fitness")
