rm(list = ls())
options(error = browser)
options(echo = FALSE)
library(testit)

w_sl<-matrix(data=c(1,2,0.3,1,1,4,2,0.1,1), nrow=3,ncol=3)  # 1996 payoff matrix
w_s<-matrix(data=c(1,0.7,2.2,2,1,0.9,1.2,1.8,1), nrow=3,ncol=3, byrow=TRUE) # 2001 payoff matrix
w_sl<-matrix(data=c(1,2,0.1,0.3,1,2,4,1,1), nrow=3, ncol=3)
w_s<-matrix(data=c(1,2.2,0.9,1.2,1,2,1.8,0.7,1), nrow=3, ncol=3)
py<-0.1; pb<-0.6; po<-1-py-pb   # Probability value at time zero
p<-c(py,pb,po)
generations<-30

sinervo_data<-matrix(data=c(0.326,0.568,0.106,
                            0.126,0.750,0.124,
                            0.361,0.328,  0.311,
                            0.420,0.321,  0.259,
                            0.485,0.389,  0.126,
                            0.350,0.551,	0.099,
                            0.426,0.452,	0.122,
                            0.603,0.150,	0.247,
                            0.524,0.374,	0.102,
                            0.452,0.460,	0.088),
                     ,nrow=10, ncol=3, byrow=TRUE)


# ITERATION EQUATION

mendelian_alleles<-function(p,w_given,generations){
  p_alleles<-c(p[1]+0.5*p[2], p[3]+0.5*p[2])
  w_phenotype<-w_given%*%p
  w_alleles<-c(p_alleles[1]*w_phenotype[1]+p_alleles[2]*w_phenotype[2], p_alleles[2]*w_phenotype[3]+p_alleles[1]*w_phenotype[2])
  w_bar<-sum(p_alleles*w_alleles)
  results<-matrix(0, nrow=generations, ncol=5)
  results[1,]<-c(p,p_alleles)

	for(i in 2:generations)
  {
    p_alleles<-(p_alleles*w_alleles)/w_bar
    w_alleles<-w_alleles*p_alleles/w_bar
    w_phenotype<-w_given%*%p
    w_bar<-sum(p_alleles*w_alleles)
    p<-c(p_alleles[1]^2, 2*p_alleles[1]*p_alleles[2], p_alleles[2]^2)
    results[i,]<-c(p,p_alleles)
  }
  return(results)
}

results_sl<-mendelian_alleles(p,w_sl,generations)  # CONFRONT USING FIRST MATRIX 1996
results_s<-mendelian_alleles(p,w_s,generations) # CONFRONT USING SECOND MATRIX 2001
results_sl
library(klaR)  # Library to plot simplex
par(mfrow=c(1,2))
triplot(results_sl[,c(3,2,1)],label=c("O","B","Y"),pch=20,col=4, main="1996 fitness")
trilines(results_sl[,c(3,2,1)], col=4)
tripoints(sinervo_data[,c(3,2,1)], pch=20,col=2)
trilines(sinervo_data[,c(3,2,1)], col=2)

triplot(results_s[,c(3,2,1)],label=c("O","B","Y"),pch=20,col=4, main="2001 fitness")
trilines(results_s[,c(3,2,1)], col=4)
tripoints(sinervo_data[,c(3,2,1)],pch=20,col=2)
trilines(sinervo_data[,c(3,2,1)], col=2)